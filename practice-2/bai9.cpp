...
double y[NUM_INPUTS];
double delta = 0.000001;    // (5 - (-5)) / NUM_INPUTS
...
void precalc() {
    ...
    *****************/
    y[0] = -5.0;
    sigmoid[0] = 1.0 / (1.0 + exp(-y[0]));;
    for(int i = 1 ; i < NUM_INPUTS ; i++) {
        y[i] = y[i-1] + delta;
        sigmoid[i] = 1.0 / (1.0 + exp(-y[i]));;
    }
}
...
inline double sigmoid_fast(double x) {
    ...
    *****************/
    if(x < y[0])    return 0.0;
    if(x > y[NUM_INPUTS - 1])   return 1.0;
    int index = (x - y[0]) / delta;
    return sigmoid[index] + ((x + 5 - index * delta) * (sigmoid[index+1] - sigmoid[index])) / delta;
}
